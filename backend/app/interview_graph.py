...
_FALLBACK_NODE_SPECS = (
    (
        "opening",
        "确认项目背景、业务目标和个人职责边界",
        "你当时为什么要做「{project}」？你主要负责哪一块？",
        ("业务目标", "个人职责", "团队边界"),
    ),
    (
        "project",
        "确认项目输入输出和关键使用场景",
        "这个项目平时怎么用？输入什么，最后产出什么？",
        ("输入输出", "使用场景"),
    ),
    (
        "architecture",
        "验证核心组件、数据流和状态流",
        "能从头到尾讲讲「{project}」是怎么跑起来的吗？哪些部分是你亲手做的？",
        ("组件职责", "数据流", "边界"),
    ),
    (
        "challenge",
        "验证故障定位、可靠性和恢复方式",
        "这个项目里最难搞的一次问题是什么？你当时怎么查出来、解决的？",
        ("故障案例", "定位过程", "恢复方式"),
    ),
    (
        "tradeoff",
        "验证方案取舍、约束和替代方案",
        "当时为什么最后选这个做法？还考虑过别的方案吗？",
        ("备选方案", "约束", "取舍原因"),
    ),
    (
        "evidence",
        "验证效果、稳定性和成本的量化方法",
        "项目上线后效果怎么样？你用什么办法确认它真的变好了？",
        ("指标", "基线", "验证方法"),
    ),
)
