...
5. 创建面试后仍然是 8 道题，并按类型连续分组：第 1～3 题为项目题，第 4～6 题为固定 Agent 基础题，第 7～8 题为固定可靠性题；每一类的第一题为该类锚题，即第 1、4、7 题。
...
保持 `POST /api/sessions` 的请求不变。服务端优先读取 Profile 下已确认的 3 道项目题，按以下位置连续分组生成会话题目：

| 题序 | 类别 | 来源 | 锚题 |
| --- | --- | --- | --- |
| 1 | project | 用户确认的项目题 1 | 是 |
| 2 | project | 用户确认的项目题 2 | 否 |
| 3 | project | 用户确认的项目题 3 | 否 |
| 4 | agent | 固定题 1 | 是 |
| 5 | agent | 固定题 2 | 否 |
| 6 | agent | 固定题 3 | 否 |
| 7 | reliability | 固定题 1 | 是 |
| 8 | reliability | 固定题 2 | 否 |
